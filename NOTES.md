# Design notes — Cozy Night (Omarchy Kids theme)

> **This theme reuses the `nord` palette verbatim and adds only OldJobobo's kid-safe wallpapers.** Same rationale ships in every theme in this batch, so the thinking travels with any repo we share.
...
His kid wallpapers turned out to be **kid-friendly reimaginings of motifs Omarchy already ships as themes** — so the matching palettes already exist and are well-tuned. Reinventing them would only create drift and maintenance duplicates. This theme is a clear example: `cozy-nightscape` echoes Nord's own night-window wallpaper, and `nighthawk-cocoa` is a cuddly, kid-safe take on Nord's *Nighthawks* — a warm cocoa counter in place of Hopper's lonely diner.
...
| `cozy-nightscape`, `nighthawk-cocoa` | **cozy-night** *(this repo)* | `nord` | nord's night-window & *Nighthawks* |
| `oh-my-ethereal` | **cozy-night** *(this repo, flagged)* | `nord` | really an `ethereal`-palette piece |
...
2. **`oh-my-ethereal`:** its cosmic look really belongs to Omarchy's `ethereal` palette, not Nord. Keep it in this Nord-based theme, move it to an `ethereal`-based kids theme, or drop it? (`last-horizon.png` and `sunset-adventure.png` were held back from v1 for the same reason — one duplicates the existing `last-horizon` theme, the other is a warm outlier.)
